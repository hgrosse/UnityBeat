Shader "Aubergine/Object/BaseFX/Fire-Pro" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(v2f i) : COLOR {
				...
				fixed4 grab = tex2Dproj(_GrabTexture, UNITY_PROJ_COORD(i.uvGrab) + n.rgrg * (i.uvGrab.y * _UvOffset + _UvMulti));
				//return fixed4(base.rgb, alpha) * (grab * 1.0 - alpha);
				return fixed4(lerp(grab.rgb, base.rgb, alpha), 1);
			}
			ENDCG 
		}
	}
